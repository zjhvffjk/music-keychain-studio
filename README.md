...
输入歌手名，**一次查询**同时拿到热门曲目与全部专辑：走热门出播放界面类成品，走全部专辑则抓全量，产出**专辑卡**（方形版式）与**专辑墙**（全部封面拼版总览）。
...
| **迷你CD 打印拼版** | 做**实体迷你唱片机**：把专辑排成 A4 300dpi 拼版（盘面 Ø40 / 封面折件 82×41 / 封底条 **111.2×38** = 右侧封4.4+封底49+左侧封4.4+左侧封背面4.4+内盘底49），带裁切线 + 50mm 校验尺。**只有封面也能跑** —— 缺的部件由**封面衍生设计引擎**自动生成，并自动联网取**真实曲目列表**印到封底 | `tools/make_minicd.py` |
...
| 作品库 | 唯一的「做过的」入口：品类筛选（全部 / 商品图 / 迷你CD设计）+ 批量选择 + 一键移入回收站（可捞回） | `workbench/server.py` |
| 歌手搜索合并 | 搜一次歌手 → 热门 30 首与全部专辑一起回来，再选出口；两条下游各自出独立任务 | `workbench/index.html` |

播放界面的每个元素（导航、封面、标签、歌名、进度条、控制按钮）都是**程序化绘制**的，
不依赖字体图标，也不依赖任何截图底图，因此分辨率可以任意缩放。

### 新增：一个搜索入口，两个出口（v1.11.0）

「歌手热门」和「专辑全集」原来各占一个页签、各要填一遍歌手名、各查一次 ——
现在是**一个「歌手」页签**：填一次歌手名点「查询」，热门 30 首和该歌手的
**全部专辑一起回来**，然后选一条路走。

| 出口 | 走哪条链路 | 产出 |
|---|---|---|
| ① 按热门出图 | 原「歌手热门」（一行没改） | 播放界面 + 可选钥匙扣 / 白底商品图 / 黑胶 |
| ② 抓全部专辑 | 原「专辑全集」（一行没改） | 封面母版 + 专辑卡 + 专辑墙 |

两条路**各自生成一个独立任务，互不影响**。

- 数据源是**两条互不依赖的路**：热门那条按「网易云主唱命中率」决定用网易云还是
  自动切 QQ 音乐（版权歌手如周杰伦，网易云命中率只有 27%，不切就会拿到一堆
  他给别人写的歌和 Live 合唱）；专辑列表只有网易云有。
  所以**一边挂了另一边照常可用** —— 网易云专辑接口限流时，热门 30 首仍能正常出图，
  界面会如实提示「专辑列表这次没拉到，点它会重试」。
- 歌手卡上的专辑张数取的是**专辑列表实际拿到的张数**（`albumCount`），
  **不是**歌手对象的 `albumSize` —— 后者在 QQ 源上恒为 0（版权歌手的歌手对象
  根本没有这个字段），照它显示会变成「0 张专辑」。
- 接口层面：`GET /api/artist?...&albums=1` 在原有返回体上**只增不改**
  （新增 `albumList` / `albumCount` / `albumError`）。不传 `albums=1` 时返回体与
  旧版逐字节一致，老脚本不受影响。专辑那一段失败也**只失败自己**，主查询照常 200。

### 新增：作品库（唯一入口 + 批量选择 + 一键回收）（v1.11.0）

「我做过什么」只有**一个**入口：右上角**作品库**。新任务一落盘就会出现在里面
（`/api/jobs` **以磁盘为准**，所以关掉工作台再打开，历史作品仍在）。

- **品类筛选**：全部 / 商品图 / 迷你CD设计。两个库的任务号可能撞号，所以选择状态
  是按 **`品类:任务号`** 记的，并且**切品类自动清空选择** —— 不清就会出现
  「看得见的没勾、看不见的勾着」，一点回收连隐藏的那几条一起搬走。
- **批量选择**：行内勾选框，或点标题文字区；工具条带 `全选当前筛选` / `反选` /
  `清空选择`，全选框有**半选**状态（如实显示 3/35）。
- **一键移入回收站**：选中后点「移入回收站（N）」。
  - 是**移入回收站**（磁盘上 `_trash/`），不是删除，能捞回。
  - **逐条报账**：批量返回 `moved[]` / `failed[]`。搬 8 条成功 7 条时，界面必须
    看得出来剩了一条 —— 只回一句 ok 的话，剩下那条会永远赖在库里而你以为删干净了。
    所以有失败会直接弹窗。
  - 正在跑的任务（`running`）拒绝回收（409）；单条删除的接口形状与旧版**完全一致**。
...
| ③ 盒体展开 | **111.2×38mm** = 右侧封 4.4 + 封底 49 + 左侧封 4.4 + 左侧封背面 4.4 + 内盘底 49 |
...
工作台顶部有三个模式页签：**歌手**、**单曲指定**、**上传封面**。

- **歌手**（一个入口，两个出口）：填歌手名 → 点「查询」。这一步**同时**把热门 30 首和
  该歌手全部专辑拿回来（只查不下载），然后选一条路走：
  - **① 按热门出图**：取热门前 5/10/15/20 首，每首出播放界面 + 可选的钥匙扣 / 白底商品图 / 黑胶。
  - **② 抓全部专辑**：把该歌手的**全部专辑**（含单曲 / EP / 演唱会 Live / Remix）一次抓下来，
    每张产出**封面母版原图**（方形，最大 2000×2000）+ **专辑卡**，最后拼一张**专辑墙**总览。
    右侧会铺开专辑网格，点任意封面就地进设计工作台，也可勾选多张批量出稿。
  - 专辑卡可选边长 **1200 / 1500 / 2000**；「最多抓几张」填 `0` 表示**全部**（周杰伦实测 44 张）。
  - 打包 ZIP 里除图片外还带一份 `albums.json`，**可离线重出**（不用再联网抓一遍）。
  - 专辑列表只走网易云（QQ 音乐没有公开的专辑列表接口），与热门那条**互不依赖**。
- **单曲指定** / **上传封面**：按歌出图的老路子，未变。

> 生成过的任务都在右上角**作品库**里（品类筛选 + 批量选择 + 一键移入回收站）。
...
│   ├── e2e_album.py           端到端验收：专辑全集（39 项断言）
│   ├── e2e_design.py          端到端验收：封面衍生设计引擎（35 项断言）
│   ├── e2e_design2.py         端到端验收：衍生设计引擎 v2（71 项断言）
│   ├── e2e_lyrics.py          端到端验收：歌词抓取与清洗（52 项断言）
│   ├── spec_minicd.py         迷你CD 包装尺寸**唯一真源**（mm，合成与打印共用）
│   ├── typo.py                排版层（字体角色 / 字距 / 折行 / ℗ 矢量绘制）
│   ├── make_minicd_sheet.py   A4 300dpi 拼版（裁切线 + 折线 + 50mm 校验尺 + PDF）
│   ├── make_minicd_board.py   迷你CD 整案设计板（8 格 + 设计思路）
│   ├── make_album_compare.py  专辑全集对照图
│   ├── audit_covers.py        封面取景审计（避开封面自带的标题字）
│   └── start_bg.py            脱离会话后台启动工作台
├── workbench/                 网页工作台
│   ├── server.py              HTTP 后端（仅标准库，只绑 127.0.0.1）
│   ├── index.html             单页前端
│   ├── design.html            迷你CD 设计工作台（可 iframe 内嵌）
│   ├── design_service.py      设计工作台后端（尺寸 / 拼版 / 印刷档案）
│   ├── art_director.py        设计语言推断（mood / style）
│   ├── packaging_*.py         包装工坊（独立工作台，首页未挂载）
│   └── _selftest.py           工作台自检
├── config/
